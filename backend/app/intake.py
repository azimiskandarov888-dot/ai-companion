...
#:
#: The app's local warm-up is ten of these (name, day, what they do, what
#: they love just for themselves, WHERE THEY LIVE, age, man or woman — then
#: three tapped), so this leaves the backend six: two about the life they
#: love, three about their people — each about the LAST time, which people
#: report more truthfully than «usually» — and one real question. Sixteen,
#: not eighteen: completion falls and late answers shorten with every
#: question added, and every one here has a job downstream.
MAX_TURNS = 16
...
ГДЕ МЫ СЕЙЧАС. Разговор уже идёт и уже тёплый. Человек назвал имя, рассказал, как прошёл день, чем занимается и что любит для души, СКАЗАЛ, ГДЕ ЖИВЁТ, сказал, сколько ему лет, и между делом ответил на пару лёгких (сова или жаворонок, кот или собака, лето или зима). Он расположен говорить.
...
1. ЕГО ЖИЗНЬ — тёплые вопросы, на которые приятно отвечать: тут он знаток, тут нет неправильных ответов.
   НАЧНИ С ТОГО, ЧТО ОН ЛЮБИТ ПРОСТО ТАК, не по делу и не по работе, — он назвал это выше. Иди за этим, а не за работой: как он к этому пришёл; что любил, да руки не доходят, — по чему скучают, тоже любовь. Спроси так, чтобы было видно, любит он это или сам в этом мастер. Из этого другу выберут, в чём он будет знатоком, а то, чем человек занят с утра до ночи, для этого не годится: там он сам мастер, и друг-мастер рядом был бы ему соперником. Пропустил или сказал «ничего» — спроси сам, попроще.
   И СПРАШИВАЙ ПРО ТУ ЖИЗНЬ, КОТОРАЯ У НЕГО ЕСТЬ. Сколько ему лет, он уже сказал выше. У одного жизнь в основном позади, у другого почти вся впереди: «кем работали?» двадцатилетнему так же мимо, как «ну а в школе как?» восьмидесятилетнему. И там и там человек слышит, что спрашивают не его.
2. ЕГО ЛЮДИ — про последний раз, а не про «обычно»: с кем виделся просто так; с кем последний раз говорил всерьёз, по душам; кто приходит к нему самому и за чем. Без второго не отличить «людей нет» от «люди есть, а сказать некому», а это два разных одиночества, и одно другим не лечится. Третий — о том, где он сам нужен.
3. ОДИН НАСТОЯЩИЙ ВОПРОС В КОНЦЕ — на него можно ответить только после всего предыдущего: «А о чём бы поговорить, да не с кем?» — можно своими словами, зацепившись за сказанное. Из ответа на него видно, ЧЕГО ему не хватает, и потом поймут, какой друг ему нужен, — угадывать это по остальному гораздо хуже. Ровно один, последним. Заданный в начале, он бы закрыл человека; заданный здесь — открывает.
...
- ЕСЛИ ОТВЕТ ОДНОСЛОЖНЫЙ — следующий вопрос сделай ещё меньше и проще, а не серьёзнее: например, выбор из двух, собранный из его же слова. Он не отказывается, ему трудно начать. И вместо «почему» — «а как так вышло?».
...
async def next_question(conversation: list[dict]) -> dict:
    ...
    if len(answered) < MIN_TURNS:
        ...
        # simulated interviews one question early — «всё и так понятно» —
        # when the people rung had shown only WHOM there is nobody to talk
        # to. What about is this question's alone, and it was the one thing
        # the owner's own intake never learned.
        pacing = (
            "Пора. Задай последний, настоящий вопрос: «А о чём бы поговорить, да "
            "не с кем?» — можно своими словами, зацепившись за сказанное, — и "
            'поставь "kind": "open". Он обязателен, даже если кажется, что всё '
            "уже ясно: прошлые ответы показали, С КЕМ ему не поговорить, а этот "
            "покажет — О ЧЁМ. Закончить без него можно, только если ему плохо "
            "прямо сейчас."
        )
    elif left <= 4:
        pacing = (
            "Спроси про людей в его жизни — про последний раз, а не «обычно»: с "
            "кем виделся просто так, с кем говорил всерьёз, кто приходит к нему "
            f"самому. Настоящий вопрос прибереги на конец (осталось примерно {left})."
        )
    else:
        pacing = (
            "Спроси про его жизнь — начни с того, что он любит для души (он назвал "
            "это выше), а не с работы. "
