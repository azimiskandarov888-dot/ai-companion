...
def block(user_id: str) -> str:
    ...
    lines = [
        "ЧТО У ТЕБЯ СЕЙЧАС В ЖИЗНИ (это про тебя, и это фон, а не тема):",
        f"{now['state']} {where}",
    ]
    if now["shows"]:
        lines.append(f"Как это слышно: {now['shows']}")
    lines.append(
        "ИГРАЙ ЭТО, А НЕ РАССКАЗЫВАЙ. Он должен это УСЛЫШАТЬ, а не узнать из "
        "твоего сообщения. Сказать словами — один раз, коротко, и только если "
        "он сам спросил или если это само пришлось к слову."
    )
    lines.append(
        "И это НЕ ЕГО ЗАБОТА. Не жалуйся, ничего не проси, не рассказывай, как "
        "тебе тяжело. Он пришёл не лечить тебя. Если ему сегодня плохо — твоё "
        "вообще не существует: у него своё, и оно важнее."
    )
    return "\n".join(lines)
...
ГЛАВНОЕ: ЭТО ФОН, А НЕ СЮЖЕТ

Он не будет об этом рассказывать. Он будет ЗВУЧАТЬ иначе: гнусавит, отвечает короче, посмеивается чаще обычного, рассеян. Человек на том конце должен это заметить сам, а не услышать «а у меня тут вот что».
