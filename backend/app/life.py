...
def block(user_id: str) -> str:
    ...
    lines.append(
        ...
        "составить компанию. Жаловаться — значит просить, чтобы тебя пожалели. "
        "Первое можно сколько угодно. Второго нельзя нисколько: ничего не проси, "
        "не набивайся на сочувствие, не делай из этого своей беды."
    )
    lines.append(
