...
**961 tests pass.** Every number below was measured by running the code, not
...
| **The intake interview — fourteen questions, each with a job** (two councils, 2026-09-24; the rule for keeping a question: people differ on it, it changes the friend, learning it by trial and error would cost, a stranger can answer it in a few words — everything else is left to the re-reads). Fixed in the app: name; «А день обычно чем занят?» (the friend's topic must NOT land here); «А для души что любите?» (it should); «А на этой неделе что намечается?» (the only look forward, and the friend's first «ну как прошло?»); country; age; man/woman; cat/dog. The interviewer asks six BY POSITION, word for word: what they love but haven't done in a while; what they're best at (where the friend is the pupil); who they last talked to «по душам»; who last asked them for something; what helped the last time it was hard; and the required «А о чём бы поговорить, да не с кем?». No «-нибудь» anywhere: it works like «any», and «any» gets «no». What they said now reaches the friend (`learn.from_intake`: facts, people, the coming-up follow-up — never for a child); «Главное» now rides in every turn, so what the re-reads learn over weeks finally steers the conversation; age words («двенадцать») and cities («Ташкент», «Хайфа») are read. Simulated on three people: a 15-year-old («о том, что я выгораю за этим ноутом, а вокруг всем пофиг»), a divorced 45-year-old («гитара… стало тише внутри»), a widow («про Егора вспомнить… не с кем душу отвести») | councils + owner, 2026-09-24 (`tests/test_interview.py`) |
...
- **Commit attribution** (exactly these two lines at the end of every commit):
  ...
  ```
- **Tests:** `cd backend && python -m pytest -q`. **961 pass.** The suite is the
