...
**965 tests pass.** Every number below was measured by running the code, not
...
| **The intake interview — every question is the interviewer's.** A semi-structured interview: the server keeps a list of twelve targets (name; what fills the day — the friend's topic must NOT land there; what they love; what is coming up this week; country; age; what they love but haven't done in a while; what they're best at; who they last talked to «по душам»; who last asked them for something; what helped the last time it was hard; «А о чём бы поговорить, да не с кем?»), each with the best wording two councils found. The model (Opus 5 — #1 on EQ-Bench 4, compared against six others on the real prompt) asks every one in the flow, reacting first, with ONE follow-up allowed when an answer is empty or too alive to pass («ничем» → «А вчера, например, как прошёл?»), and one gentle rescue for a vague last answer. The client hands back each question's target; the server overrules a model that wanders off the list or stops early, so country and age cannot be skipped. The owner rejected the earlier fixed warm-up on first use — «ничем» went unanswered and it felt like a form — and he was right: probing chatbots get more informative answers than fixed surveys (Xiao 2020), follow-up questions make an asker liked (Huang 2017). Also: what they said reaches the friend (`learn.from_intake`), «Главное» rides in every turn, age words and cities are read. Simulated end to end on the owner's own case: 14 questions, ~3 s a question | owner + councils, 2026-09-24 (`tests/test_interview.py`, `tests/test_intake.py`) |
...
| | Part | Model | Notes |
|---|---|---|---|
| ⑧ | **Interviewer** — the intake conversation | Opus 5 | asks every question, reacts and follows up; chosen by comparison (§4) |
...
- **Commit attribution** (exactly these two lines at the end of every commit):
  ...
  ```
- **Tests:** `cd backend && python -m pytest -q`. **965 pass.** The suite is the
